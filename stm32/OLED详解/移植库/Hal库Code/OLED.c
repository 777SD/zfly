...
#define OLED_W_SCL(x)		HAL_GPIO_WritePin(GPIOA, SCL_Pin, (BitAction)(x))
#define OLED_W_SDA(x)		HAL_GPIO_WritePin(GPIOA, SDA_Pin, (BitAction)(x))
...
/**
  ...
  * @param  Char 要显示的 ???????个字符，范围：ASCII可见字符
  * @retval  ???????
  */
void OLED_ShowChar(uint8_t Line, uint8_t Column, char Char)
{
	...
	for (i = 0; i < 8; i++)
	{
		OLED_WriteData(OLED_IIC_F8x16[Char - ' '][i]);			//显示上半部分内容
	}
	OLED_SetCursor((Line - 1) * 2 + 1, (Column - 1) * 8);	//设置光标位置在下半部 ???????
	for (i = 0; i < 8; i++)
	{
		OLED_WriteData(OLED_IIC_F8x16[Char - ' '][i + 8]);		//显示下半部分内容
	}
}
...
void OLED_ShowChinese(uint32_t Line,uint32_t Column,uint32_t Number)
{
	...
	for(i = 0;i<16;i++)
	{
		OLED_WriteData(Font_Chinese[2*Number][i]);
	}
	OLED_SetCursor((Line-1)*2+1,(Column-1)*16);
	for(i =0;i<16;i++)
	{
		OLED_WriteData(Font_Chinese[2*Number+1][i]);
	}
}
...
void OLED_Init(void)
{
	...
	OLED_WriteCommand(0x40);	//设置显示 ???????始行
